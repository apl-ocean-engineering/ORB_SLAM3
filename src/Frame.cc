...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, const std::shared_ptr<ORBextractor> &extractorLeft, const std::shared_ptr<ORBextractor> &extractorRight, const std::shared_ptr<ORBVocabulary> &voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, const std::shared_ptr<GeometricCamera> &pCamera, Frame* pPrevF, const IMU::Calib &ImuCalib)
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, const std::shared_ptr<ORBextractor> &extractor,const std::shared_ptr<ORBVocabulary> &voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, const std::shared_ptr<GeometricCamera> &pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(nullptr),
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, const std::shared_ptr<ORBextractor> &extractor,const std::shared_ptr<ORBVocabulary> &voc, const std::shared_ptr<GeometricCamera> &pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(nullptr),
     mTimeStamp(timeStamp), mK(dynamic_cast<Pinhole*>(pCamera.get())->toK()), mK_(dynamic_cast<Pinhole*>(pCamera.get())->toK_()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
...
void Frame::UndistortKeyPoints()
{
    ...
    // Undistort points
    mat=mat.reshape(2);
    cv::undistortPoints(mat,mat, dynamic_cast<Pinhole*>(mpCamera.get())->toK(),mDistCoef,cv::Mat(),mK);
    mat=mat.reshape(1);
...
void Frame::ComputeStereoFishEyeMatches() {
    ...
    for(vector<vector<cv::DMatch>>::iterator it = matches.begin(); it != matches.end(); ++it){
        if((*it).size() >= 2 && (*it)[0].distance < (*it)[1].distance * 0.7){
            ...
            float depth = dynamic_cast<KannalaBrandt8*>(mpCamera.get())->TriangulateMatches(mpCamera2,mvKeys[(*it)[0].queryIdx + monoLeft],mvKeysRight[(*it)[0].trainIdx + monoRight],mRlr,mtlr,sigma1,sigma2,p3D);
            if(depth > 0.0001f){
