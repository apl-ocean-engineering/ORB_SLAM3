...
namespace ORB_SLAM3 {
    ...
    loopClosing_(true), sensor_(sensor) {
        // if(bNeedToRectify_){
    ...
    loopClosing_(true), sensor_(sensor) {
        ...
        if(sensor_ == SensorType::IMU_MONOCULAR || sensor_ == SensorType::IMU_STEREO || sensor_ == SensorType::IMU_RGBD){
            readIMU(fSettings);
    ...
    void Settings::readCamera1(cv::FileStorage &fSettings) {
        ...
        if (cameraModel == "PinHole") {
            ...
            if((sensor_ == SensorType::MONOCULAR || sensor_ == SensorType::IMU_MONOCULAR) && vPinHoleDistorsion1_.size() != 0){
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        if(found){
            ...
            if(!bNeedToRectify_){
                ...
                if((sensor_ == SensorType::STEREO || sensor_ == SensorType::IMU_STEREO) && cameraType_ != Rectified){
            ...
            if(!bNeedToRectify_){
                ...
                if((sensor_ == SensorType::STEREO || sensor_ == SensorType::IMU_STEREO) && cameraType_ != Rectified){
                    ...
                    if(cameraType_ == KannalaBrandt){
                        dynamic_cast<KannalaBrandt8*>(calibration1_.get())->mvLappingArea[0] *= scaleColFactor;
                        dynamic_cast<KannalaBrandt8*>(calibration1_.get())->mvLappingArea[1] *= scaleColFactor;

                        dynamic_cast<KannalaBrandt8*>(calibration2_.get())->mvLappingArea[0] *= scaleColFactor;
                        dynamic_cast<KannalaBrandt8*>(calibration2_.get())->mvLappingArea[1] *= scaleColFactor;
                    }
                }
            }
        }
    ...
    void Settings::readOtherParameters(cv::FileStorage& fSettings) {
        ...
        loopClosing_ = static_cast<bool>(readParameter<int>(fSettings,"System.loopClosing", found, true));
    }
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        if((settings.sensor_.isStereo()) && (settings.cameraType_ !=  Settings::Rectified)){
        ...
        if(settings.bNeedToRectify_){
            ...
            if((settings.sensor_ == SensorType::STEREO || settings.sensor_ == SensorType::IMU_STEREO) &&
        ...
        if(settings.sensor_ == SensorType::STEREO || settings.sensor_ == SensorType::IMU_STEREO){
            ...
            if(settings.cameraType_ == Settings::KannalaBrandt){
                auto vOverlapping1 = dynamic_cast<KannalaBrandt8*>(settings.calibration1_.get())->mvLappingArea;
                auto vOverlapping2 = dynamic_cast<KannalaBrandt8*>(settings.calibration2_.get())->mvLappingArea;
        ...
        if(settings.sensor_ == SensorType::IMU_MONOCULAR || settings.sensor_ == SensorType::IMU_STEREO || settings.sensor_ == SensorType::IMU_RGBD) {
        ...
        if(settings.sensor_ == SensorType::RGBD || settings.sensor_ == SensorType::IMU_RGBD){
