...
public:
  enum Value : uint8_t
  {
    ...
    SensorType() = delete;
    constexpr SensorType(Value stype) : value(stype) { }

    constexpr bool operator==(SensorType a) const { return value == a.value; }
    constexpr bool operator!=(SensorType a) const { return value != a.value; }

    constexpr bool isImu() const { return (value == IMU_MONOCULAR || value == IMU_RGBD || value==IMU_STEREO); }
    constexpr bool isRGBD() const { return (value == RGBD || value == IMU_RGBD); }
    constexpr bool isStereo() const { return (value == STEREO || value==IMU_STEREO); }
    constexpr bool isMonocular() const { return (value == MONOCULAR || value==IMU_MONOCULAR); }
